...
async function main() {
    const serialize = (v) => JSON.stringify(v, (_, val) => typeof val === 'bigint' ? Number(val) : val, 2);
  ...
  `;
    ...
    const testContracts = await prisma.$queryRaw `
  ...
  `;
    ...
    const rejectedWithData = await prisma.$queryRaw `
