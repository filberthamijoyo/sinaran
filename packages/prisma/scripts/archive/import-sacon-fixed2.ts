...
function mapPipelineStatus(acc: string | null, proses: string | null): string {
  const a = String(acc || '').trim().toUpperCase();
  const p = String(proses || '').trim().toUpperCase();

  if (a === 'TIDAK ACC') return 'REJECTED';
  if (p === 'BATAL')     return 'REJECTED';
  if (p === 'PENDING')   return 'PENDING_APPROVAL';
  if (p === 'MENUNGGU')  return 'PENDING_APPROVAL';
  if (a === 'ACC' && p === 'PROSES') return 'COMPLETE';
...
async function importSacon() {
  ...
  console.log(`  SACON import complete: ${imported} imported, ${skipped} skipped, ${errors} errors`);
}
...
async function main() {
  ...
  const minMax = await prisma.$queryRaw<[{min: Date, max: Date}]>`SELECT MIN(tgl) as min, MAX(tgl) as max FROM "SalesContract"`;
  console.log('\n=== Final Counts ===');
