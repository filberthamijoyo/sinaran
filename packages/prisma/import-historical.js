...
function isOnOrAfter2025(d) {
    return d !== null && d >= CUTOFF;
}
function parseSaconDate(val) {
    if (!val)
        return null;
...
async function importSacon() {
    ...
    for (let i = 2; i < data.length; i++) {
        ...
        try {
            ...
            errors++;
            if (errors <= 5)
                console.error(`  Row ${i + 1} KP=${kp}: ${e.message}`);
        }
    }
...
async function importIndigo() {
    ...
    for (let i = 2; i < data.length; i++) {
        ...
        if (!tanggal || !isOnOrAfter2025(tanggal)) {
            skipped++;
            continue;
        }
        try {
...
async function importWeaving() {
    ...
    const rows = (0, sync_1.parse)(csvRaw, {
        columns: true,
    ...
    for (const row of rows) {
        ...
        try {
            ...
            errors++;
            if (errors <= 5)
                console.error(`  KP=${kp} ${tanggal} shift=${shift} machine=${machine}: ${e.message}`);
        }
    }
