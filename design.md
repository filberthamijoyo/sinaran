...
Here is the updated prompt with your new color palette integrated seamlessly into the Material You design system rules, replacing the old hex codes and adjusting the color names to match your tokens.
...
Material You uses a sophisticated tonal palette. For this implementation, use a **Blue seed** (#0061A4) and integrate the following specific design tokens:

**Core Palette Structure**:
- **Card Backgrounds**: `#E8F1F8` - Soft, cool tinted surface for containers (replaces pure white)
- **Primary Text**: `#181C20` - Near-black with slight coolness for main content
- **Secondary Text**: `#41474D` - Dark gray-blue for supporting text
- **Muted Labels**: `#71787E` - Medium gray for disabled states and metadata
- **Accent / Primary**: `#0061A4` - Deep blue (seed color)
- **Danger / Red**: `#BA1A1A` - For errors and destructive actions
- **Success / Green**: `#006E1C` - For positive feedback and success states
- **Warning / Amber**: `#7D5700` - For cautionary states
- **Borders**: `#C1C7CE` - Light gray-blue for subtle boundaries
- **On Primary**: `#FFFFFF` - Pure white for text on primary elements

**Color Relationship Rules**:
- Use the Card Background tone (#E8F1F8) to create depth hierarchy instead of stark white surfaces.
- Accent/Primary color should appear in CTAs, focus states, and key interactive elements.
- Secondary Text and Muted Labels provide typographic hierarchy without relying on font-weight alone.
- Danger, Success, and Warning are reserved for semantic state communication.
- Never use pure white (#FFFFFF) for card backgrounds - always use the tinted Card Background color.
- On colored backgrounds (Accent/Primary), use transparent white/black overlays for states.
...
- When needed, use `#C1C7CE` (Borders) color
...
- Color: Accent/Primary, Success, or Warning at 10-30% opacity
...
- Use accent, success, and warning colors at 80-90% opacity
- Apply `blur-3xl` for soft, atmospheric quality
- Position partially off-canvas (using negative translate values)

**Background Treatment**:
- Never use solid white - always use the Card Backgrounds color (#E8F1F8)
- Radial gradients for subtle color washes: `bg-[radial-gradient(circle_at_top_right,_var(--color-md-accent)_0%,_transparent_40%)]`
...
2. **Tonal (Secondary)**:
   - Background: Card Backgrounds color (#E8F1F8)
   - Text: Primary Text color (#181C20)
   - Shape: `rounded-full`
...
3. **Outlined**:
   ...
   - Border: 1px Borders color (#C1C7CE)
...
**Visual Treatment**:
- Background: Card Backgrounds (`#E8F1F8`), never pure white
...
**State Transitions**:
- Hover: `hover:bg-md-primary/5` or `hover:shadow-md`, combined with transforms
...
- Bottom border: 2px solid Borders color
- Background: Card Backgrounds color (#E8F1F8)
- Height: Tall (`h-14` / 56px)
- Focus state: Bottom border changes to Accent/Primary color
...
└─────────────┘  ← Square bottom with 2px border
      ↑
  Focus: Accent/Primary color
```

**State Handling**:
- Rest: `border-md-borders` (bottom)
- Focus: `border-md-primary` (bottom)
- Transition: 200ms color transition
- Label: Placeholder uses `text-md-muted`
...
1. **Organic Blur Shapes with Layering**:
   ...
   - Use accent, success, warning colors at 10-30% opacity
...
2. **Tonal Surface System with Shadow Progression**:
   ...
   - Layer surfaces: Base Background → Card Backgrounds
   - Color difference is subtle but creates depth without heavy shadows
   - All cards use the Card Backgrounds color by default
...
**Don't:**
- ❌ Use pure white (#FFFFFF) for card backgrounds - breaks the tonal system
...
- ❌ Use pure black text - use the Primary Text color (#181C20)
...
- Text on card background: 4.5:1 minimum (Primary Text color: #181C20)
- Text on Primary: AAA level (pure white #FFFFFF)
- Borders color: 3:1 against surfaces
...
- [ ] Card backgrounds use #E8F1F8 (not pure white)
- [ ] Cards use Card Backgrounds (#E8F1F8)
